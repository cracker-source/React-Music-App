...
interface PopularReleasesProps {
    artistInfo: IndividualArtistData | undefined;
}

const PopularReleases = ({ artistInfo }: PopularReleasesProps) => {
    const [swiperInstance, setSwiperInstance] = useState<any>();
    return (
        <Fragment>
            <div className="flex items-center justify-between">
                <h4 className="font-semibold text-3xl">Popular Releases</h4>
                <div>
                    <button
            ...
            <div className="mt-5">
                <Swiper
                    ...
                    modules={[Navigation]}
                    className="mySwiper">
                    {artistInfo?.discography?.popularReleases?.items.map(
                        (item: Item7, index) => {
                            return (
                                <SwiperSlide>
...
export default memo(PopularReleases);
